		...
		for (let patIdx = 0, patNum = libopenmpt._openmpt_module_get_num_patterns(this.modulePtr); patIdx < patNum; patIdx++) {
			...
			for(let rowIdx = 0, rowNum = libopenmpt._openmpt_module_get_pattern_num_rows(this.modulePtr, patIdx); rowIdx < rowNum; rowIdx++) {
				...
				for (let chIdx = 0; chIdx < chNum; chIdx++) {
					// PATCH (tracker): emit the full cell formatted exactly as a
					// tracker renders it ("C-4 01 v64 A04") instead of 6 raw command
					// values — libopenmpt owns the per-format note/effect quirks.
					// Returns a malloc'd string we must free. Runs once per load
					// (not in the audio render path).
					const cellPtr = libopenmpt._openmpt_module_format_pattern_row_channel(this.modulePtr, patIdx, rowIdx, chIdx, 0, 0)
					row.push( libopenmpt.UTF8ToString(cellPtr) )
					libopenmpt._openmpt_free_string(cellPtr)
				}

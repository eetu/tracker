...
> **Tokens are still provisional.** The full halo `colors_and_type.css` has not
> been adopted yet — a provisional dark palette + theme tokens live in
> `src/routes/+layout.svelte` (`--bg/--panel/--accent/…` plus a `--surface-*`
> set for the player). Light/dark/auto themes key off `data-theme` on `<html>`
> (`src/lib/theme.svelte.ts`). When the halo pass happens, remap these tokens to
> `--halo-*`; until then, consume the local tokens, never hard-coded hex.
...
under the playhead (ties to the `ScanLine` centerline-toggle metaphor). Sources:
...
(`--font-retro` = Amiga **TopazPlus**), coloured `--accent` (halo amber,
`#f0a02a`). It's the sole header brand — no tagline.
